...
# Create FastAPI app
app = FastAPI(title="Cybersecurity KPI Suggestion API (Simplified)",
              description="Simplified API for generating suggestions for Application Owners based on their security metrics")
...
# Function to generate suggestions based on AO metrics (simplified version)
    ...
    for template_data in suggestion_templates:
        if template_data["condition"](metrics):
            try:
                ...
                # Instead of using ML ranking, just shuffle and take top 5
                print(f"Error formatting suggestion: {e}")
    random.shuffle(applicable_suggestions)
    top_suggestions = applicable_suggestions[:5]
...
@app.get("/")
async def root():
    return {"message": "Simplified Cybersecurity KPI Suggestion API is running"}
